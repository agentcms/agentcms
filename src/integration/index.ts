...
/**
 * A generated entry for one of the package's API routes: it re-exports the route and installs the
 * integration's runtime config, which the route reads at request time. `page-ssr` never runs for
 * a .ts endpoint, so without this an endpoint in an isolate that has not rendered a page yet saw
 * no config at all -- no inline `site`, no `basePath`, no custom binding names.
 */
export function renderApiRouteModule(entrypoint: string, config: Record<string, unknown>): string {
  return `// Generated by @agentcms/agentcms. Do not edit.
export * from "${entrypoint}";

globalThis.__AGENTCMS_CONFIG__ ??= ${JSON.stringify(config, null, 2)};
`;
}
  ...
  return {
    ...
    hooks: {
      "astro:config:setup": async ({
        ...
        // The config every AgentCMS route reads at runtime. Pages get it from `page-ssr`;
        // endpoints get it from the generated entries below.
        const runtimeConfig = {
          mode,
          basePath: base || "/blog",
          postsPerPage,
        ...
        let apiDir: URL | undefined;
        try {
          const { mkdir } = await import("node:fs/promises");
          apiDir = createCodegenDir();
          await mkdir(apiDir, { recursive: true });
        } catch (err) {
          apiDir = undefined;
          logger.warn(
            `Could not generate the API route entries (${err instanceof Error ? err.message : String(err)}); ` +
              "the API will see the inline site config and basePath only after a page has rendered."
          );
        }
        const injectApi = async (pattern: string, entrypoint: string) => {
          if (apiDir) {
            try {
              const { writeFile, rename } = await import("node:fs/promises");
              const name = `api${pattern.replace(/[^a-z0-9]+/gi, "-")}.mjs`;
              const target = new URL(name, apiDir);
              // Temp then rename, as for the SEO modules: concurrent dev rebuilds can truncate.
              const temp = new URL(`${name}.${process.pid}.tmp`, apiDir);
              await writeFile(temp, renderApiRouteModule(entrypoint, runtimeConfig), "utf-8");
              await rename(temp, target);
              injectRoute({ pattern, entrypoint: target });
              return;
            } catch {
              // fall through to the package route
            }
          }
        ...
        // ---------------------------------------------------------------
        await injectApi("/api/agent/publish", "@agentcms/agentcms/routes/api/publish.ts");
        await injectApi("/api/agent/posts", "@agentcms/agentcms/routes/api/list.ts");
        await injectApi("/api/agent/posts/[slug]", "@agentcms/agentcms/routes/api/post.ts");
        await injectApi("/api/agent/context", "@agentcms/agentcms/routes/api/context.ts");
        await injectApi("/api/agent/upload", "@agentcms/agentcms/routes/api/upload.ts");
        ...
        if (publicApi) {
          ...
          if (!owned.has("posts")) {
            await injectApi("/api/posts", "@agentcms/agentcms/routes/api/public-posts.ts");
            await injectApi("/api/posts/[slug]", "@agentcms/agentcms/routes/api/public-post.ts");
            injected.push("/api/posts", "/api/posts/[slug]");
          }
          if (!owned.has("tags")) {
            await injectApi("/api/tags", "@agentcms/agentcms/routes/api/public-tags.ts");
            injected.push("/api/tags");
          }
          if (!owned.has("categories")) {
            await injectApi("/api/categories", "@agentcms/agentcms/routes/api/public-categories.ts");
        ...
        // ---------------------------------------------------------------
        await injectApi("/images/[...path]", "@agentcms/agentcms/routes/images.ts");
        ...
        // Endpoints get the same object from their generated entries (see injectApi above).
        injectScript(
          "page-ssr",
          `globalThis.__AGENTCMS_CONFIG__ = ${JSON.stringify(runtimeConfig)};`
        );
